...
/* JWT:WE DON'T HAVE THIS!: # include <sys/stropts.h>	** for I_PUSH */
...
/* JWT:WE DON'T HAVE THIS!:  #if defined (__svr4__) || defined (__lnx21__) */
#if 0
/*
...
#endif
	else if (ctrl) {  /* JWT:HANDLE OUR Ctrl-key SEQUENCES: */
	    switch (keysym) {
...
/*        to give proper Scroll behaviour */
	...
	selection_paste(ev->xselection.requestor, ev->xselection.property, False);
	break;
	...
	if (ev->xany.window == TermWin.vt) {
		...
		case Button2:
		    selection_request(ev->xbutton.time,
					ev->xbutton.x, ev->xbutton.y, (ev->xbutton.state & ShiftMask)
					? aterm_XA_CLIPBOARD : XA_PRIMARY);  /* JWT:Shift+Button2: PASTE CLIPBOARD! */
		    break;
