...
/* JWT:HAVE TO KEEP A SEPARATE CLIPBOARD BUFFER (OTHERWISE SELECT OVERWRITES)!: */
unsigned char *cbsel_text = NULL;
int cbsel_len = 0;
...
/* ------------------------------------------------------------------------- */
/*
 * Request the current primary selection be pasted in
 * EXT: button 2 release
 */
/* PROTO */
void
selection_request(Time tm, int x, int y, Atom selbuffer)
{
    selection.request_time = tm;
    if (selbuffer == XA_PRIMARY && XGetSelectionOwner(Xdisplay, selbuffer) == None) 
    ...
    else 
	{
		...
		if (selbuffer == aterm_XA_CLIPBOARD && cbsel_len > 0)
			PasteIt(cbsel_text, cbsel_len);		/* internal selection */
		else
			XConvertSelection(Xdisplay, selbuffer, XA_STRING, selbuffer, TermWin.vt,tm);
    }
}
...
selection_clear(Atom selbuffer)
{
	...
	if (selbuffer == aterm_XA_CLIPBOARD) {  /* JWT:CLEARS OUR "CLIPBOARD": */
        cbsel_len = 0;
        if (cbsel_text != NULL) {
            FREE(cbsel_text);
            cbsel_text = NULL;
	...
	/* JWT:(ELSE) CLEAR THE PRIMARY SELECTION: */
    if (selection.text)
        FREE(selection.text);
 ...
 */
    ...
    *str = '\0';
    ...
    if (key_state & (Mod1Mask | Mod3Mask))
    {
        ...
        if (i > 0) {
            cbsel_text = MALLOC(i * sizeof(char) + 1);
            strcpy((char *) cbsel_text, (char *) new_selection_text);
        }
    }
    else
...
selection_to_clipboard()
{
    ...
    cbsel_len = selection.len;
    if (cbsel_text != NULL) {
        FREE(cbsel_text);
        cbsel_text = NULL;
    }
    if (selection.len > 0) {
        cbsel_text = MALLOC(selection.len * sizeof(char) + 1);
        strcpy((char *) cbsel_text, (char *) selection.text);
    }
}
...
selection_send(XSelectionRequestEvent * rq)
{
    ...
    if (rq->target == aterm_XA_TARGETS) {
		...
		cl[0] = (rq->selection == aterm_XA_CLIPBOARD && cbsel_len > 0)
				? cbsel_text : selection.text;  /* JWT:(OUR) CLIPBOARD, OR PRIMARY. */
